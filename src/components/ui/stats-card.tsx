      ...
      <Card className={cn('w-full max-w-sm overflow-hidden', className)} ref={ref} {...props}>
        ...
        <CardContent>
          ...
          <div className={`flex items-center gap-1 text-sm ${changeColor}`}>
            ...
            <span>{Math.abs(changeValue)}% <span className="text-muted-foreground">{changeDescription}</span></span>
          </div>

          <div className="mt-6 h-32 w-full">
            <AnimatePresence>
              <motion.div key="chart" variants={containerVariants} initial="hidden" animate="visible" className="flex h-full w-full items-end justify-between gap-2">
                {chartData.map((point) => (
                  <div key={point.label} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                    <div className="relative flex h-full w-full items-end justify-center gap-1.5">
                      <motion.div custom={point.currentValue} variants={barVariants} className={cn('w-full rounded-sm bg-primary', primaryBarClassName)} aria-valuenow={point.currentValue} aria-label={`Current value: ${point.currentValue}`} role="progressbar" />
                      <motion.div custom={point.previousValue} variants={barVariants} className={cn('w-full rounded-sm bg-muted', secondaryBarClassName)} aria-valuenow={point.previousValue} aria-label={`Previous value: ${point.previousValue}`} role="progressbar" />
                    </div>
